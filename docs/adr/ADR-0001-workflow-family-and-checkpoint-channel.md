...
**Question-TUI routing investigated (2026-09-06, parked):** routing host-side rendering through third-party question-TUI extensions is not feasible on pi 0.85.1 — no programmatic tool-invocation API exists on `ExtensionAPI` (verified against the installed `dist/core/extensions/types.d.ts:906–1085`), and earendil-works/pi#3497 requesting it was auto-closed by bot 11s after creation and explicitly declined by the maintainer ("no fix... use the event bus... no plans of implementing it") — its `COMPLETED` state is bot boilerplate, not completion. The event-bus workaround requires cooperating extensions, which arbitrary question TUIs are not. `ctx.ui` dialogs are therefore the only extension rendering path; this feature is parked until pi ships programmatic tool calling.

**Published (2026-09-06):** the extension ships to npm as `pi-checkpoint-bridge` (OIDC trusted publishing via `.github/workflows/release-publish.yml`, gated on `paths_released`). It is **required for all `workflows/openspec/` authoring workflows** — `openspec-plan-change` probes for it at resolve time and returns a friendly `checkpoint-bridge-not-installed` error (never a crash) when absent. Install: `pi install npm:pi-checkpoint-bridge`.
